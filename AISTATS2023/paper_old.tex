...
Accumulated Local Effects (ALE) is a popular explainable AI method that quantifies how a feature influences the decisions of a model, handling well correlated features. However, in case of strong interactions between features, instance-level feature effects may deviate from the average curve provided by ALE. Therefore, it is crucial to quantify this deviation, namely, the uncertainty of the effect. In this work, we define Uncertainty-aware ALE (UALE) to quantify and visualize, on a single plot, both the average effect of a feature and the associated uncertainty. Furthermore, as in ALE, UALE's approximation requires partitioning the feature domain into non-overlapping intervals (bin-splitting) based on which the average effect and uncertainty are then computed from the instances that lie in each bin.  In this work, we formally prove that to achieve an unbiased estimation of the uncertainty in each bin, bin-splitting must follow specific constraints and moreover, we propose a method for automatically finding the optimal intervals, balancing the trade-off between estimation bias and variance~\todo{check if this phrase explains well the concept.} We demonstrate through synthetic and real datasets (a) the advantages of modeling the uncertainty with UALE compared to other methods\todo{I think we compare to 1 other method} and (b) the importance of appropriate bin splitting for an accurate estimate of the average effect and uncertainty.
\end{abstract}


\section{INTRODUCTION}
Machine Learning (ML) is been adopted nowadays in critical domains,
such as healthcare, education and finance. In these areas, we need a combination
...
partitioning that secures an unbiased estimate of the
uncertainty.
...
\color{blue}\todo{This should go to related work, probably sec 2.2}In contrast, in PDP the
...
background for ALE is presented in
Section~\ref{sec:feat-effect-meth}, the limitations in Section~\ref{sec:quant-heter-effects} whereas ALE
approximations are presented
in Section~\ref{sec:ale-approximation}.

\paragraph{Notation} 
\todo{Should be checked again: d is the cardinality, D is the dimensionality, smt should be wrong with the feature vector then}
\todo{We should also add the $$\mathbf{X}_c$ symbol used hereafter but not introduced here. }
...
PDP
formulates the FE of the \(s\)-th attribute as an expectation over the \todo{Kati ginetai edo}
>>>>>>> origin/overleaf-2022-10-11-1029
marginal distribution \(\mathbf{X}_c\), i.e.,
\(f^{\mathtt{PDP}}(x_s) =
\mathbb{E}_{\mathbf{X}_c}[f(x_s,\mathbf{X}_c)]\), whereas MP
formulates it as an expectation over the conditional \todo{Should Xs here also be in bold?}
\(\mathbf{X}_c|X_s\), i.e.,
\(f^{\mathtt{MP}}(x_s) = \mathbb{E}_{\mathbf{X}_c|x_s}[f(x_s,
\mathbf{X}_c)]\). ALE defines the local effect of the \(s\)-the feature
...
local effects \todo{add: accumulation over++++}:
...
the local effects \todo{add what local means here, maybe instance-
level feature effects?} deviate from the
global explanation, i.e.~we need to quantify the uncertainty of the average
effect.
...
%Both approximations, however, require the number of bins \(K\) as input and perform an equally-sized bin splitting of the attribute domain. 
...
The choice of \(K\) is therefore critical and introduces

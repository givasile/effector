...
\(f(\mathbf{x}) = 4x_1^2 + x_2^2 + x_1x_2\), so the effect is
\(f^{\mathcal{GT}}_{\mu}(x_1) = 4 x_1^2\) and the uncertainty
\(f^{\mathcal{GT}}_{\sigma}(x_1) = \sigma_2 x_1\). Due to the
non-linear effect, wide bins increase the mean residual error,
\(\mathcal{L}^{\rho}\) making the uncertainty approximation
biased. This can be clearly observed in in the bottom-right of
Figure~\ref{fig:ex-synth-2-2}. On the other hand, narrow bins lead to
worse approximation due to the limited number of
samples. Interestingly, UALE's bin splitting manages to compromise
these conflicting objectives and provide an accurate estimation, as we

...
\aistatstitle{Accumulated Regional Effects: An ALE-based method for regional effects with uncertainty quantification}
...
\begin{abstract}
  This paper is on Feature Effect (FE), a class of global explainability techniques that create a plot with the average effect of a single feature on the output. The task is particularly challenging when the black-box function contains non-linear interactions between correlated features. In these cases, FE should provide, first, the expected effect and, second, the uncertainty of the effect, i.e.~the level of heterogeneous effects due to implicit feature interactions. Accumulated Local Effects (ALE) is a novel method that models accurately the expected effect in cases of correlated features, overcoming limitations of previous approaches. In this paper, we define Accumulated Regional Effects (ARE) a FE method that extends ALE to quantify the heterogeneous effects. We also provide an alternative definition of ARE based on a sequence of interpretable variable-size regions and we formally prove that under some conditions both definitions are equivalent. Using that, we convert the typical bin-splitting step of ALE to a principled optimization problem and we present an algorithm for finding the optimal sequence of regions. We demonstrate through synthetic and real-datasets, that ARE quantifies correctly the average and heterogeneous effects and approximates them robustly by extracting regions with similar-effect.
\end{abstract}
...
\subsection{Interval-Based Formulation of ARE}
...
zero, when \(h \to 0\). We define the region-effect and the
region-uncertainty inside the region \([z_1, z_2)\) as:
...
%
We also define as \(\mathcal{Z}_s\) the sequence of \(K+1\) points
that partition the \(s\)-th feature axis into \(K\) variable-size
intervals, i.e. \(\mathcal{Z}_s = \{z_0, z_K\}\), where
\(z_0=x_{s,min}\) and \(z_K=x_{s,max}\). Based on the above, we
redefine ARE using an interval-based formulation
\(\tilde{f}^{\mathtt{ARE}}(x_s):=
\tilde{f}^{\mathtt{ALE}}_{\mu}(x_s) \pm
\tilde{f}^{\mathtt{ALE}}_{\sigma}(x_s)\), where:
...
\textit{If we define the residual \(\rho(z)\) as the difference
  between the expected effect at \(x_s\) and the region-effect, i.e
  \(\rho(z) = \mu(z) - \mu(z_1, z_2)\), then, the region-variance
  \(\sigma^2(z_1, z_2)\) equals to the integration of the point-wise
  variances plus the squared residuals inside the interval:
\begin{equation}
 \sigma^2(z_1, z_2) = \frac{\int_{z_1}^{z_2} \sigma^2(z) + \rho^2(z) \partial z}{z_2 - z_1}
\end{equation}}
%
...
\textit{If we define as \(\rho^2(z_{k-1}, z_{k})\) the
  region-residual, where
  \(\rho^2(z_{k-1}, z_{k}) = \int_{z_{k-1}}^{z_k} \rho^2(z)\partial
  z\), then the two definitions of ARE,
  \(f^{\mathtt{ARE}}, \tilde{f}^{\mathtt{ARE}}\) are equivalent, if
  and only if the sequence of limits \(\mathcal{Z}_s\) are such that
...
\(\mathcal{H}(z_1, z_2) = \int_{z_1}^{z_2}\mathcal{H}^2(z) \partial z\),
...
ARE explanation as in the original definition. In general, we use the
quantity \(Q_{\mathcal{Z}} = \sum_{k=1}^K \rho^2(z_{k-1}, z_{k})\) to
measure the dissimilarity between the actual definition of ARE
\(f^{\mathtt{ARE}}\) and the region-based definition
\(\tilde{f}^{\mathtt{ARE}}\) using the sequence of intervals
\(\mathcal{Z}_s\). In this way, we access a quantified metric for
evaluating how good is a particular splitting.
...
% bin is under the danger of adding nuisance uncertainty.

Both approximations are unbiased only under the assumption that the
points are uniformly distributed inside the intervals. We set-up the
...
minimizes the sum of bin-costs. The cost of each bin is the mean
variance \(\hat{sigma}^2_k\) scaled by the bin length \(\Delta z_k\)
...
\begin{figure}[h]
  ...
  \caption{With interaction, equal weights: Feature effect for all
    ...
    (a) ARE method at the left columns and (b) PDP-ICE at the right
    column.}
...
For evaluating ARE in terms of bin-splitting we set-up the following
...
ARE approximation with all possible fixed-size alternatives, in terms

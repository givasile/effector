...
% indicating the particular method in use.\footnote{An extensive list of
%   all symbols used in the paper is provided at the Appendix.}
...
by evaluating the black box-function at the bin limits:
\begin{equation}
...
(DALE) that uses the partial derivatives:
\begin{equation}
...
\end{equation}
%
Their method has the advantages of remaining on-distribution when
bins become wide and, most importantly, allows the recomputation of
the accumulated effect with different bin-splitting with near-zero
computational cost.

Both approximations ask from the user to blindly decide the bin
density for splitting the feature axis into \(K\) equisized bins, an
approach that is error prone. Setting \(K\) to a small value may hide
fine-grain effects due to large bins while setting \(K\) to a high
value leads to poor bin-effect estimations due to few samples per

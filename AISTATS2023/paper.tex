...
have been criticized~\citep{Gromping2020MAEP} to erroneously compute
the effects when the input features are correlated.
...
indicate no effect on the output or a highly positive effect for some
instances and a highly negative for others. For resolving such
ambiguities, FE methods should quantify and visualize the
heterogeneity of instance-level explanations. In the case of PDP, this
heterogeneity is visualized via the Individual Conditional
Explanations (ICE)~\citep{goldstein2015peeking}. However ICE plots do
have the same limitations with PDP in case of correlated features. To
the best of our knowledge, no such method has been proposed for ALE.

% Such ambiguities could be resolved if the heterogeneity of instance-level feature effects is provided in addition to the average effect. 
% In this paper, we measure the heterogeneity as the standard deviation of the instance-level effects and we refer to it using as the \emph{uncertainty of the effect}.
...
(local) effects and is quantified by their standard deviation. As with
ALE, UALE's approximation requires partitioning the feature domain
into non-overlapping intervals (bin-splitting). However, as we
formally prove, a fixed equi-width partitioning without considering
...
propose an automatic uncertainty-driven partitioning that secures an
...
(Section~\ref{sec:feat-effect-meth}) and the heterogeneity
(Section~\ref{sec:quant-heter-effects}). In Section, we provide the
necessary background for ALE definition
(Section~\ref{sec:feat-effect-meth}) and approximation
(Section~\ref{sec:ale-approximation}).
...
\emph{Aggregated Local Effects} (ALE).  PDP formulates the global FE
as an expectation over the marginal distribution \(\mathbf{X}_c\),
i.e.,
\(f^{\mathtt{PDP}}(x_s) =
\mathbb{E}_{\mathbf{X}_c}[f(x_s,\mathbf{X}_c)]\), whereas MP as an
expectation over the conditional \(\mathbf{X}_c|X_s\), i.e.,
\(f^{\mathtt{MP}}(x_s) = \mathbb{E}_{\mathbf{X}_c|x_s}[f(x_s,
\mathbf{X}_c)]\).

ALE defines the local effect of the \(s\)-th feature at a specific
point \((z, \xc)\) of the input space \(\mathcal{X}\) with the partial
derivative:
\(f^s(z, \xc) = \frac{\partial f}{\partial x_s} (z, \xc)\).
...
\subsection{Interval-Based Formulation}

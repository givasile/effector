...
\end{equation}
%
In Theorem~\ref{sec:theorem-1}, though, we prove that
...
\(\sigma^2_*(z_1, z_2) = \frac{\int_{z_1}^{z_2} \mathbb{E}_{\xc|x_s=z}
  ...
  z_1}\) which, in the general case, overestimates
\(\sigma^2(z_1, z_2)\).
...
In Theorem \ref{sec:theorem-1}, we show that the estimation of the
uncertainty is unbiased only when \(\mathcal{E}^2(z_1, z_2) = 0\).

% \(\mathcal{E}^2(z_1, z_2) = 0\).
% Theorem \ref{sec:theorem-1} show an important connection between
% the uncertainty we want to measure, i.e. the average uncertainty
% inside the bin, \(\sigma(z_1, z_2)\), and the one we actual
% estimate, \(\sigma_*(z_1,z_2)\), using the available samples. For
% convenience, we denote as
% \(\mathcal{E}^2(z_1, z_2) = \frac{\int_{z_1}^{z_2}\rho^2(z) \partial
%   z}{z_2 - z_1}\) the error term that measures the models the expected square

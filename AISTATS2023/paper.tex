...
We perform a two-fold evaluation of UALE. The first group
...
of instance-based effects. The second group
...
This a qualitative example with the aim to highlight that PDP-ICE is
vulnerable to misleading estimations of both the average effect and
the uncertainty, even when the black-box function is relatatively
simple.
...
(\( a_1 \neq a_2 \)) and (c) with interaction (\(\alpha > 0\)) with
equal weights (\(a_1=a_2\)).

In the general case, there is no unique ground-truth feature effect
and uncertainty and each method defines the effect in a different
way. We select this simple case, to determine the ground-truth,
exploiting two facts. First, in \(p(\xb)\) it holds that
\(x_1=x_2\). Therefore, knowing the value of either \(x_1\) or
\(x_2\), we know the closed-form of \(f(\xb)\), too. For example, if
\(a_1=a_2=1\) and \(x_1 = \frac{1}{6}\), then
\(f_1(\xb) = \frac{1}{3}\), so
\(f(\xb) = f_1(\xb) + \alpha f_2(\xb) \). Afterwards, we use the
properties of interpretable models~\citep{molnar2022} to determine the
ground-truth. In the example above, if also \(a=0\), then
\(f(\xb) = x_1 + x_2\) which is an interpretable linear model. For the
uncertainty, we use the fact that under no-interactions, \(a=0\), the
uncertainty must be zero (no heteregoneity). We discuss separately the
case with \(a>0\). Finally, we want to also show that UALE's
approximation is accurate, due to appropriate bin splitting.

\paragraph{No Interaction, Equal weights.}

In this set-up, \(\alpha=0\) (no interaction) and the \(a_1=a_2=1\)
(equal weights). We examine only the effect of feature \(x_1\),
because it is exactly the same with \(x_2\). Based on the discussion
above, the ground truth effect \(f_\mu^{\mathtt{GT}}(x_1)\) is:
\(x_1\) in \([0, \frac{1}{4}]\), \(-x_1\) in
\([\frac{1}{4}, \frac{1}{2}]\) and \(0\) in \([\frac{1}{2},
1]\). Because \(x_1\) does not interact with any other variable, the
uncertainty is \(f^{\mathcal{GT}}_{\sigma^2}(x_1) = 0\) in
\([0,1]\). In Figure~\ref{fig:synth-ex-1-case-1}, we observe that PDP
effect is wrong and ICE plots show heterogeneous effects. In contrast,
UALE models correctly both the average effect and the absence of

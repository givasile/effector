...
(XAI), to understand the decision mechanism of complex black-box
...
A popular class of global methods are the so-called Feature Effect (FE) methods~\citep{Gromping2020MAEP} that isolate the impact of a single feature on
the output\footnote{FE methods also isolate the combined effect of a
  ...
  visualize.}. There are three widely-used FE methods: \emph{Partial Dependence
...
The necessary background for ALE is presented in Section~\ref{sec:feat-effect-meth}), whereas ALE approximations are presented in Section~\ref{sec:ale-approximation}. 
...
FE methods reply to the question \textit{what is the average (global)
  ...
  extent the local effects \todo{add what local means here} deviate from the global (average)
  effect/explanation}. 
  \todo{A connector line is needed here on the connection between local-global and uncertainty}
  Quantifying the uncertainty of the global explanation
...
interaction between the input features, with an interaction value \todo{not clear what htis value is, maybe add a few words}. A
...
each bin by evaluating the black box-function $f()$ at the bin limits: \todo{k or K? I prefer k for the cardinality and it seems that k is used afterwards. }
\begin{equation}
...
Both approximations, however, require the number of bins \(K\) as input and perform an  equally-sized bin splitting of the attribute domain. The choice of \(K\) is therefore critical and introduces significant limitations. Specifically, 
setting \(K\) to a small

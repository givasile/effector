...
\begin{figure}[h]
  ...
  \includegraphics[width=.23\textwidth]{real_dataset_3/feature_4_ale_auto.pdf}
  \includegraphics[width=.23\textwidth]{real_dataset_3/feature_4_pdp_ice.pdf}\\
...
As in the Syntetic Example (Case 2), we treat as ground-truth the
...
close to the ground truth. Afterwards, we randomly select \(N=1000\)
samples and we compare UALE approximation with all possible fixed-size
alternatives using \(\mathcal{L}^{\mu}, \mathcal{L}^{\sigma}\). Add discussion
...
\begin{figure}[h]
  ...
  \includegraphics[width=.23\textwidth]{real_dataset_3/compare_mu_err_feature_4.pdf}
  \includegraphics[width=.23\textwidth]{real_dataset_3/compare_var_err_feature_4.pdf}\\

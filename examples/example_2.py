...
#                            uncertainty=True)
# if savefig:
#     pathname = os.path.join(path, "example_2", "dale_fixed_bins.pdf")
...
alg_params = {"bin_method" : "dp", "max_nof_bins" : 30, "min_points_per_bin": 10}
dale.fit(features="all", alg_params=alg_params)
y, var, stderr = dale.eval(x=np.linspace(axis_limits[0,0], axis_limits[1,0], 100),
                           s=0,
                           uncertainty=True)
if savefig:
    pathname = os.path.join(path, "example_2", "dale_feat_0.pdf")
    dale.plot(s=0, error="std", savefig=pathname)
    pathname = os.path.join(path, "example_2", "dale_feat_1.pdf")
    dale.plot(s=1, error="std", savefig=pathname)
    pathname = os.path.join(path, "example_2", "dale_feat_2.pdf")
...
pdp_ice = fe.PDPwithICE(data=X,
                        ...
                        axis_limits=axis_limits)
if savefig:
    pathname = os.path.join(path, "example_2", "pdp_ice_feat_0.pdf")
    pdp_ice.plot(s=0, normalized=True, nof_points=300, savefig=pathname)
    pathname = os.path.join(path, "example_2", "pdp_ice_feat_1.pdf")
    pdp_ice.plot(s=1, normalized=True, nof_points=300, savefig=pathname)
    pathname = os.path.join(path, "example_2", "pdp_ice_feat_2.pdf")

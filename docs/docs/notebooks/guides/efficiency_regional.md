...
def measure_time(method_name, features, model, N, M, D, repetitions, K, model_jac=None,):
    ...
    # Regional* classes were removed: build the GLOBAL effect, then query find_regions.
    method_map = {
        "pdp": effector.PDP,
    ...
    for _ in range(repetitions):
        ...
        # fit = global fit + the region search (find_regions), matching the old regional .fit
        tic = time.time()
        method.fit(**fit_kwargs)
        partitions = {feat: method.find_regions(feat, finder=finder) for feat in features}
        fit_time_list.append(time.time() - tic)

        # eval on the root region (node_idx == 0): mean effect + heterogeneity, model-free
        tic = time.time()
        for feat in features:
            part = partitions[feat]
            part.eval(0, xx, centering=True)
            part.eval_heter(0, xx)

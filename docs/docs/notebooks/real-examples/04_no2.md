...
```
    ...
    I0000 00:00:1783956205.498386   67798 cpu_feature_guard.cc:227] This TensorFlow binary is optimized to use available CPU instructions in performance-critical operations.
...
```
    ...
    ============================================================
    data: 400 instances × 7 features (7 continuous) — target no2_concentration
    model output: mean -0.0118, std 0.779, range [-2.23, 1.55] · R² 0.656 (on this subsample)
    explained variance: global effects (GAM) 89.3%
      + split wind_speed (on cars_per_hour, wind_direction) → 93.3% (+4.0 pts, heter 0.193→0.098)
      + split temperature_diff_2m_25m (on cars_per_hour, wind_direction, wind_speed) → 95.5% (+2.2 pts, heter 0.158→0.117)
      rejected: cars_per_hour (on hour_of_day, temperature_at_2m, temperature_diff_2m_25m) — -0.8 pts, redundant (variance already explained)
      rejected: wind_direction (on cars_per_hour, temperature_diff_2m_25m, wind_speed) — -1.3 pts, redundant (variance already explained)
    ------------------------------------------------------------
    feature                   importance     heter  #regions
    ------------------------------------------------------------
    cars_per_hour                 0.7413    0.1999         1
    temperature_at_2m             0.2547    0.1472         1
    wind_speed                    0.2443    0.0977         7
    temperature_diff_2m_25m       0.2076    0.1166         7
    ============================================================
    the plotted features carry 83% of the total importance mass
...
```
    ...
    [effector] global effects reproduce 87.6% of the model's variance; with subregions, 95.5%
    ...
    [effector] global effects reproduce 86.9% of the model's variance; with subregions, 95.5%
    ...
    [effector] global effects reproduce 90.2% of the model's variance; with subregions, 95.3%

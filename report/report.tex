...
Generalized Additive Models (GAMs)~\citep{hastie1987generalized} are a popular class of explainable by design (x-by-design) models~\citep{rudin2019stop, ghassemi2021false}.
Their popularity stems from their inherent interpretability.
GAMs represent an aggregation of univariate functions, where the overall model can be expressed as \(f(\xb) = c + \sum_{s=1}^D f_s(x_s)\).
...
However, GAMs encounter limitations when predicting outcomes that depend on multiple features simultaneously.
In such cases, the unknown predictive function includes terms representing interactions between features.
To mitigate this limitation, GA$^2$Ms~\citep{lou2013accurate} extended GAMs adding pairwise interactions in the
formulation, i.e., \(f(\xb) = c + \sum_{s=1}^D f_s(x_s) + \sum_{s=1}^D \sum_{s_1 \neq s_2} f_{s_1 s_2}(x_{s_1}, x_{s_2})\).
Interpreting GA$^2$Ms involves visualizing both the first-order ($1D$ plots) and second-order (($2D$ plots)) components.
However, as the number of features increases, the number of pairwise interactions grows exponentially,
making it impractical for users to remember and interpret a large number of two-dimensional plots.
Therefore, these methods target on automatically selecting the most significant interaction terms.

Both GAMs and GA$^2$Ms have limitations in modeling interactions beyond the second degree,
...
The main challenge lies in the difficulty of visualizing more than three terms simultaneously,
...
RAMs tackle the challenge of visualizing general interaction terms involving more than two variables by focusing
on a specific class of terms.
These terms have a general structure of these terms is
...
on a combination of three features: $x_1 \in [20, 80]$ (age), $x_2 \in [0, 40]$ (years in work), and $x_3 \in \{True, False\}$ (married).
Both GAM and GA$^2$M would fail to capture the effect of the three-feature interaction.
However, the three-feature effect can be decomposed in two sets of second-degree conditional terms based on the marital status:
$f_{1}(x_1, x_2 | x_3 = True)$
and
$f_{2}(x_1, x_2 | x_3 = False)$, which can be visualized and interpreted as using two-dimensional plots.
In a different scenario, it may be better to it in four sets of first-degree conditional terms:
...
The objective of RAMs is to automatically identify the most significant conditional terms and fit a GAM for each of them.
The RAM framework involves three steps.
First, a black-box model is fitted to capture all high-order interactions.
Then, the subregions where the black-box model exhibits near-local additivity are identified using Regional Effect Plots (REPs).
Finally, a GAM is trained specifically for each identified subregion.
...
\SetKwInOut{Output}{Output}
\BlankLine
\Input{Dataset $(X, y)$, Black box model $f$, Maximum depth $T$}
\BlankLine
\Output{Subregions $\mathcal{R}_{sT_s}$ for all features $s$, where $T_s \leq 2^T$}
\BlankLine
\tcc{Needed for DALE feature effect estimation}
Compute the Jacobian w.r.t. features $\xb$,  $J = \nabla_{\xb}f(\xb)$ \;
\For{$s = 1$ to $D$}{
  \For{$t = 1$ to $T$}{
    \tcc{Find best split feature $c$ at point $p$, leading to loss
      $\mathcal{H}_{st}$ using regions of previous level}
    $\mathcal{H}_{st}, c, p$ $\gets$
    BestSplit($\left\{\mathcal{R}_{sk}|k\in\left\{1,\ldots,T_{t-1}\right\}\right\}$)\;
    \If{$\lVert\mathcal{H}_{st} - \mathcal{H}_{s(t-1)}\rVert < \epsilon$}{
      break\;
    }
  }
}

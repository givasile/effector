...
\For{$s = 1$ to $D$}{
    \For{$c \in \{1, \dots, D\} \setminus \{s\}$}{
        ...
        J\_list $\gets [J]$\;
        \For{$t = 1$ to $T$}{
            best\_split $\gets$ BestSplit(X\_list, J\_list, s)\;
            optimal\_splits[s, c, t] $\gets$ best\_split\;
            X\_list $\gets$ best\_split["X\_list"]\;
            J\_list $\gets$ best\_split["J\_list"]\;
        }
    }
}
\end{algorithm}
...
\Input{X\_list, J\_list, s, c}
\Output{BestSplits}

% Initialize variables
\BlankLine
\uIf{c is categorical}{
    positions $\gets$ GetUniqueValues(X\_list, c)\;
    L $\gets$ []\; % \Comment{List to store interaction values}
    \For{p \textnormal{in} positions}{
        X\_split, J\_split $\gets$ SplitDatasetCategorical(X\_list, J\_list, c, p)\;
        L $\gets$ GetInteraction(X\_split, J\_split)\;
    }
    split\_position = argmin(L)
}
\Else{
    positions $\gets$ GetPosition(X\_list)\;
    L $\gets$ []\; % \Comment{List to store interaction values}
    \For{p \textnormal{in} positions}{
        X\_split, J\_split $\gets$ SplitDatasetNumerical(X\_list, J\_list, c, p)\;
        L $\gets$ GetInteraction(X\_split, J\_split)\;
    }
    split\_position = argmin(L) % \Comment{Store the minimum interaction value for the categorical feature}
}
...
%\For{c \in \{1, \dots, D\}}{
%    \If{c is categorical}{
%        unique\_values $\gets$ GetUniqueValues(X\_list, c)\;
%        L $\gets$ []\;
%        \For{p in unique\_values}{
%            X\_split, J\_split $\gets$ SplitDatasetCategorical(X\_list, J\_list, c, p)\;
%            L $\gets$ GetInteraction(X\_split, J\_split)\;
%        }
%        L[c] = min(L)\;
...
\begin{algorithm}
\caption{SplitDatasetCategorical}
\label{alg:split_dataset_categorical}
\SetAlgoLined
\SetKwInOut{Input}{Input}
\SetKwInOut{Output}{Output}
\Input{X\_list, J\_list, c, val}
\Output{Split datasets}
X\_split $\gets$ []\;
J\_split $\gets$ []\;
\For{i = 1 to len(X\_list)}{
    ind\_equal $\gets$ X[:, c] $=$ val\;
    ind\_diff $\gets$ X[:, c] $\neq$ val\;
    Append X[ind\_equal], X[ind\_diff] to X\_split\;
    Append J[ind\_equal], J[ind\_diff] to J\_split\;
}
...
J\_split $\gets$ []\;
\For{i = 1 to len(X\_list)}{
    ind\_left $\gets$ X[:, c] $\leq$ val\;
    ind\_right $\gets$ X[:, c] $>$ val\;
    Append X[ind\_left], X[ind\_right] to X\_split\;
    Append J[ind\_left], J[ind\_right] to J\_split\;
}
\Return X\_split, J\_split
\end{algorithm}

...
The backbone of the algorithm is described in Algorithm~\ref{alg:subregion_identification}.
The basic idea is for each feature $s \in \{1, \ldots, D\}$, to exhaustively search level-by-level, for
all possible splits before choosing the one that minimizes the interaction.
For the first level, we search
...
\BlankLine
splits[s, t] $\gets$ None\;
positions[s, t] $\gets$ None\;
feat\_c[s, t] $\gets$ None\;
\For{$s = 1$ to $D$}{
    X\_list $\gets [X]$\;
    J\_list $\gets [J]$\;
    \For{$t = 1$ to $T$}{
